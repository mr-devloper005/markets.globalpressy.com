...
function getRegisterConfig(kind: ReturnType<typeof getProductKind>) {
  ...
  if (kind === 'editorial') {
    return {
      ...
      icon: FileText,
      title: 'Create your distribution account',
      body: 'Set up your organization profile to publish press releases, manage campaigns, and scale media visibility.',
    }
  }
  if (kind === 'visual') {
    return {
        ...
        <section className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-stretch">
          ...
          <div className={`rounded-[2rem] p-8 ${config.panel}`}>
            ...
            <form className="mt-6 grid gap-4" onSubmit={handleSubmit}>
              ...
              />
              <input className="h-12 rounded-xl border border-current/10 bg-transparent px-4 text-sm" placeholder="Company or brand name" />
              <input className="h-12 rounded-xl border border-current/10 bg-transparent px-4 text-sm" placeholder="Primary distribution goal (e.g. launches, funding, brand updates)" />
              <button
                type="submit"
                disabled={isLoading}
                className={`inline-flex h-12 items-center justify-center rounded-full px-6 text-sm font-semibold disabled:opacity-60 ${config.action}`}
              >
